...
import slaService from '../../services/mock/slaService'
...
export default function BdcSlaPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      setData(await slaService.getSLAData())
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  return (
    <div className="mx-auto w-full max-w-7xl">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        />
        <StatCard
          label="Leads Within SLA"
          value={formatNumber(data.overview.withinSla)}
        />
        <StatCard
          label="Leads Near SLA"
          value={formatNumber(data.overview.nearSla)}
        />
        <StatCard
          label="Leads Outside SLA"
          value={formatNumber(data.overview.outsideSla)}
        />
      </div>
