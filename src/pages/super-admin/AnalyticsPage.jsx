...
export default function AnalyticsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      setData(await platformAnalyticsService.getPlatformAnalytics())
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  return (
    <div className="mx-auto w-full max-w-7xl">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-5">
        ...
        <StatCard label="Average Lead Score" value={kpis.averageLeadScore} />
                ...
                <div className="w-full rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--bg-muted)] px-4 py-3 text-center">
                  ...
                  {prev && (
                    <p className="text-xs text-[var(--text-muted)]">
                      Conversion: {pct(step.count, prev.count)}
                    </p>
                  )}
                </div>
